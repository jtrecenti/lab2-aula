...
format:
  html:
    ...
    theme: cosmo
execute:
  # avisos ficam à vista de propósito: foi um aviso do `tune_grid` que
  # denunciou a ordem errada dos passos da receita neste documento
  message: false
---
...
::: {.callout-important}
## O `select()` acima não é arrumação, é proteção

A tabela original tem 33 colunas, e três delas são identificadores de texto:
`trip_id`, `route_short_name` e `route_long_name`. Se elas sobreviverem até a
receita, o `recipe(headway_seg ~ .)` vai tratá-las como preditoras e criar
milhares de dummies a partir de colunas que são praticamente únicas por linha.

O sintoma aparece no `tune_grid`:

```
warning: There are new levels in `route_short_name`: "1012-10", "1016-10", ...
```

Se você vir esse aviso, o `select()` desta seção não rodou na sua sessão. Rode
o documento do início. Aviso sobre nível novo em `tipo_linha` é normal e
esperado (há linhas de trem e metrô raras); sobre `trip_id`, nunca é.
:::
...
`step_novel()` vem **antes** de `step_unknown()`. O `step_novel()` reserva um
nível para categoria que só aparece depois do treino; o `step_unknown()` cuida
do que está faltando. Na ordem trocada, o `recipes` avisa e converte o nível
novo em `NA`, que já passou do ponto de ser tratado. O ecossistema do R avisa;
vale ler o aviso.
